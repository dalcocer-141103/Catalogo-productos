...
function toggleCarrito() {
    ...
    renderizarCarrito(); // Asegurar que esté actualizado al abrir

}



function agregarAlCarrito(e, prod) {

    if(e) e.stopPropagation(); // Evitar abrir detalle si se pulsa en la grilla
    ...
    toggleCarrito(); // ABRIR EL CARRITO AUTOMÁTICAMENTE PARA CONFIRMAR
...
function actualizarCantidad(sku, cambio) {
    ...
    const item = carrito.find(p => p.sku === sku);
...
function actualizarBurbujaCarrito() {
    ...
    const bubble = document.getElementById('cart-counter-bubble');

    const totalItems = carrito.reduce((sum, item) => sum + item.cantidad, 0);

    bubble.innerText = totalItems;
...
function renderizarCarrito() {
    ...
    const contenedorCart = document.getElementById('cartItemsContainer');
    ...
    if(carrito.length === 0) {
        ...
        contenedorCart.innerHTML = '<div style="text-align:center; color:#777; margin-top:40px;"><span class="material-icons-outlined" style="font-size:40px; opacity:0.5;">shopping_bag</span><p>Tu carrito está vacío</p></div>';
    ...
    carrito.forEach(item => {
        ...
        contenedorCart.innerHTML += `
            ...
            <div class="cart-item">
                ...
                <img src="${item.imagen}" onerror="this.src='https://via.placeholder.com/50'">

                <div class="cart-info">

                    <div class="cart-title">${item.nombre.substring(0, 40)}...</div>

                    <div class="cart-controls">

                        <button class="qty-btn" onclick="actualizarCantidad('${item.sku}', -1)">-</button>

                        <span style="color:white; font-size:0.9rem;">${item.cantidad}</span>

                        <button class="qty-btn" onclick="actualizarCantidad('${item.sku}', 1)">+</button>

                        <span class="cart-price" style="margin-left:auto;">${formatearMoneda(subtotal)}</span>
    ...
    totalDisplay.innerText = formatearMoneda(totalCalculado);
...
function enviarPedidoWhatsApp() {
    ...
    let mensaje = "Hola Eritech, quiero cotizar el siguiente pedido:%0A%0A";
    ...
    carrito.forEach(item => {
        ...
        mensaje += `▪️ *${item.cantidad}x* ${item.nombre} (SKU: ${item.sku})%0A`;

    });

   

    mensaje += `%0A*Total Estimado: ${formatearMoneda(total)}*%0A`;

    mensaje += "%0A¿Me podrían confirmar existencia y envío?";
...
window.addEventListener('load', actualizarBurbujaCarrito);
...
function renderizarProductos(lista) {
    ...
    <div class="btn-card-action" style="background:var(--eri-light); color:var(--bg-body); border-color:var(--eri-light);"

         onclick="agregarAlCarrito(event, ${JSON.stringify(prod).replace(/"/g, '&quot;')})">
